  ...
  function validateAndAdd(operation, operations, checksum, index2) {
    if (isValidateOperationRange && operations.length !== 0) {
      ...
      if (lastOperation.kind === operation.kind && operation.start < lastOperation.end && operation.start > lastOperation.start) {
        ...
        throw new Error(`operation (block index: ${index2}, checksum: ${checksum}, kind: ${OperationKind2[operation.kind]}) overlaps previous operation (checksum: ${checksum}):
  ...
  if (partCount <= 1) {
    const w = (index2) => {
      ...
      if (task.kind === downloadPlanBuilder_1$1.OperationKind.COPY) {
        DataSplitter_1$1.copyData(task, out2, options.oldFileFd, reject, () => w(index2));
      } else {
    ...
    await new Promise((resolve, reject) => {
      ...
      w = (index2) => {
        ...
        if (operation.kind === downloadPlanBuilder_1.OperationKind.COPY) {
          ...
          DataSplitter_1.copyData(operation, firstStream, oldFileFd, reject, () => w(index2));
          return;
        }
...
const initDb = () => {
  ...
  return new Promise(async (resolve) => {
    const file2 = join(app2.getAppPath(), "db.json");
...
const gloabReadDbData = (key) => {
  {
    return readDbData(key);
  }
};
const gloabWriteDbData = (data) => {
  console.log("main");
  {
...
const loginOutAction = () => {
  {
    Promise.resolve().then(() => index).then((res) => {
...
const baseOptions = async (params, method = "post") => {
...
const baseRequest = (url, data, method = "post") => {
  return new Promise(async (resolve, reject) => {
...
const initIpc = (mainWindow, workWindow2) => {
  ...
  require$$1$3.ipcMain.handle("unauthorizedFetch", () => {
    return unauthorizedFetch();
  });
  require$$1$3.ipcMain.handle("loginOutFromWork", () => {
    mainWindow.webContents.send("login-out", true);
  });
};
let workWindow = null;
