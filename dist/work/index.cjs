...
const gloabReadDbData = (key) => {
  {
    return new Promise((resolve) => {
      Promise.resolve().then(() => require("./index-065ccb17.cjs")).then((res) => {
        const { readDbData: readDbData2 } = res;
        readDbData2(key).then((res2) => {
          resolve(res2);
        }).catch(() => {
          resolve("");
        });
...
const gloabWriteDbData = (data) => {
  console.log("work");
  {
    return Promise.resolve().then(() => require("./index-065ccb17.cjs")).then((res) => {
      const { writeDbData: writeDbData2 } = res;
      writeDbData2(data);
    });
  }
};
...
const axiosRequest = (option) => {
  return axios$1(option);
};
...
require$$1.ipcRenderer.on("msgFormRender", (event, msg) => {
  console.log("msgFormRender:", msg);
  if (msg === "unauthorizedFetch") {
    unauthorizedFetch();
  }
});
require$$1.ipcRenderer.on("port", (e) => {
